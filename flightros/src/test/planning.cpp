...
#include "flightros/test/planning.hpp"
...
void MotionPlanner::run(image_transport::Publisher rgb_pub) {
...
void MotionPlanner::readPointCloud() {
  ...
  std::string filepath =
    std::experimental::filesystem::path(__FILE__).parent_path().string() + "/data/warehouse_point_cloud.ply";
  try {
...
bool MotionPlanner::plan() {
  ...
  std::cout << ">>> Bound information" << std::endl;
  std::cout << ">>> min x: " << bounds_.min.x << std::endl;
  std::cout << ">>> min y: " << bounds_.min.y << std::endl;
  std::cout << ">>> min z: " << bounds_.min.z << std::endl;
  std::cout << ">>> max x: " << bounds_.max.x << std::endl;
  std::cout << ">>> max y: " << bounds_.max.y << std::endl;
  std::cout << ">>> max z: " << bounds_.max.z << std::endl;
...
void MotionPlanner::executePath(image_transport::Publisher rgb_pub) {
  // initialization
  assert(!vecs_.empty());
  ...
  rgb_camera_->setWidth(128);
  rgb_camera_->setHeight(128);
  ...
  unity_bridge_ptr_->addTracker(quad_ptr_);
