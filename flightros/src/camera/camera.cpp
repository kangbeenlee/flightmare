...
int main(int argc, char *argv[]) {
  ...
  // unity_bridge_ptr->addTracker(target);
  unity_ready = unity_bridge_ptr->connectUnity(scene_id);

  FrameID frame_id = 0;
  while (ros::ok() && unity_ready) {
    quad_state.x[QS::POSZ] = 5;
    quad_state.x[QS::POSY] = 3;
    if (quad_state.x[QS::POSZ] < 20)
      quad_state.x[QS::POSZ] += 0.01;

    std::cout << quad_state.x[QS::POSZ] << std::endl;

    quad_ptr->setState(quad_state);
    // target->setState(target_state);
