...
import os
import copy
from rpg_baselines.multi_agent.off_policy.networks import Actor, Critic_MADDPG
...
class MADDPG(object):
    def __init__(self, args, agent_id):
        ...
        self.actor = Actor(args, agent_id).to(args.device)
        self.critic = Critic_MADDPG(args).to(args.device)
        self.actor_target = copy.deepcopy(self.actor).to(args.device)
    ...
    def choose_action(self, obs, noise_std):
        with torch.no_grad():
            ...
            a = self.actor(obs).cpu().numpy().flatten()
    ...
    def save_model(self, save_dir, algorithm, time_steps, agent_id):
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        torch.save(self.actor.state_dict(), "./model_multi/{}_actor_step_{}k_agent_{}.pth".format(algorithm, int(time_steps / 1000), agent_id))
