...
#
from rpg_baselines.single_agent.common.policies import ActorCriticPolicy, RecurrentActorCriticPolicy
...
class PPO2(ActorCriticRLModel):
    ...
    def learn(self, total_timesteps, log_dir, logger, callback=None, log_interval=1, tb_log_name="PPO2", reset_num_timesteps=True):
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                try:
                    ...
                    if self.best_score == None or self.best_score < ep_reward_mean:
                        self.best_score = ep_reward_mean
                        self.best_update = update
                        self.save(log_dir + "_Iteration_{}".format(update))
                        print(f"Best update is {update}")

                    if ep_reward_mean == np.nan:
                        print("Nan score occurs")
                        sys.exit()
