...
class UnityBridge {
 public:
  ...
  bool handleOutput();
  bool getPointCloud(PointCloudMessage_t &pointcloud_msg, Scalar time_out = 600.0);
  ...
  bool addTarget(std::shared_ptr<TargetQuadrotor> target);
  bool addTracker(std::shared_ptr<TrackerQuadrotor> tracker);
 ...
 private:
  ...
  std::vector<std::shared_ptr<TargetQuadrotor>> unity_targets_;
  std::vector<std::shared_ptr<TrackerQuadrotor>> unity_trackers_;
