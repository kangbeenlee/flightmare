...
bool UnityBridge::getRender(const FrameID frame_id) {
  ...
  for (size_t idx = 0; idx < pub_msg_.targets.size(); idx++) {
    unity_targets_[idx]->getState(&quad_state);
    pub_msg_.targets[idx].position = positionRos2Unity(quad_state.p);
    pub_msg_.targets[idx].rotation = quaternionRos2Unity(quad_state.q());
  }

  for (size_t idx = 0; idx < pub_msg_.trackers.size(); idx++) {
    unity_trackers_[idx]->getState(&quad_state);
    pub_msg_.trackers[idx].position = positionRos2Unity(quad_state.p);
    pub_msg_.trackers[idx].rotation = quaternionRos2Unity(quad_state.q());
  }
...
bool UnityBridge::addTarget(std::shared_ptr<TargetQuadrotor> quad) {
  Target_t target_t;
  // get quadrotor state
  QuadState quad_state;
  if (!quad->getState(&quad_state)) {
  ...
  target_t.ID = "target" + std::to_string(settings_.targets.size());
  target_t.position = positionRos2Unity(quad_state.p);
  target_t.rotation = quaternionRos2Unity(quad_state.q());
  target_t.size = scalarRos2Unity(quad->getSize());
  ...
  for (size_t cam_idx = 0; cam_idx < rgb_cameras.size(); cam_idx++) {
    std::shared_ptr<RGBCamera> cam = rgb_cameras[cam_idx];
    Camera_t camera_t;
    camera_t.ID = target_t.ID + "_" + std::to_string(cam_idx);
    camera_t.T_BC = transformationRos2Unity(rgb_cameras[cam_idx]->getRelPose());
    camera_t.channels = rgb_cameras[cam_idx]->getChannels();
    camera_t.width = rgb_cameras[cam_idx]->getWidth();
    camera_t.height = rgb_cameras[cam_idx]->getHeight();
    ...
    camera_t.is_depth = false;
    camera_t.output_index = cam_idx;
    target_t.cameras.push_back(camera_t);

    // add rgb_cameras
    rgb_cameras_.push_back(rgb_cameras[cam_idx]);
  }
  unity_targets_.push_back(quad);

  //
  settings_.targets.push_back(target_t);
  pub_msg_.targets.push_back(target_t);
  return true;
}

bool UnityBridge::addTracker(std::shared_ptr<TrackerQuadrotor> quad) {
  Tracker_t tracker_t;
  // get quadrotor state
  QuadState quad_state;
  if (!quad->getState(&quad_state)) {
  ...
  tracker_t.ID = "tracker" + std::to_string(settings_.trackers.size());
  tracker_t.position = positionRos2Unity(quad_state.p);
  tracker_t.rotation = quaternionRos2Unity(quad_state.q());
  tracker_t.size = scalarRos2Unity(quad->getSize());
  ...
  for (size_t cam_idx = 0; cam_idx < rgb_cameras.size(); cam_idx++) {
    std::shared_ptr<RGBCamera> cam = rgb_cameras[cam_idx];
    Camera_t camera_t;
    camera_t.ID = tracker_t.ID + "_" + std::to_string(cam_idx);
    camera_t.T_BC = transformationRos2Unity(rgb_cameras[cam_idx]->getRelPose());
    camera_t.channels = rgb_cameras[cam_idx]->getChannels();
    camera_t.width = rgb_cameras[cam_idx]->getWidth();
    camera_t.height = rgb_cameras[cam_idx]->getHeight();
    ...
    camera_t.is_depth = false;
    camera_t.output_index = cam_idx;
    tracker_t.cameras.push_back(camera_t);

    // add rgb_cameras
    rgb_cameras_.push_back(rgb_cameras[cam_idx]);
  }
  unity_trackers_.push_back(quad);

  //
  settings_.trackers.push_back(tracker_t);
  pub_msg_.trackers.push_back(tracker_t);
  return true;
}
...
bool UnityBridge::handleOutput() {
  ...
  for (size_t idx = 0; idx < settings_.targets.size(); idx++)
  {
    // update vehicle collision flag
    unity_targets_[idx]->setCollision(sub_msg.sub_targets[idx].collision);

    // feed image data to RGB camera
    for (const auto& cam : settings_.targets[idx].cameras) {
      for (size_t layer_idx = 0; layer_idx <= cam.enabled_layers.size();
           layer_idx++) {
        if (!layer_idx == 0 && !cam.enabled_layers[layer_idx - 1]) continue;
  ...
  for (size_t idx = 0; idx < settings_.trackers.size(); idx++)
  {
    // update vehicle collision flag
    unity_trackers_[idx]->setCollision(sub_msg.sub_trackers[idx].collision);

    // feed image data to RGB camera
    for (const auto& cam : settings_.trackers[idx].cameras) {
      for (size_t layer_idx = 0; layer_idx <= cam.enabled_layers.size();
           layer_idx++) {
        if (!layer_idx == 0 && !cam.enabled_layers[layer_idx - 1]) continue;
