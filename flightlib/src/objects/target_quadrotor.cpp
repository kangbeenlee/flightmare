...
TargetQuadrotor::TargetQuadrotor(const std::string &cfg_path)
...
TargetQuadrotor::TargetQuadrotor(const QuadrotorDynamics &dynamics)
  : world_box_((Matrix<3, 2>() << -100, 100, -100, 100, -100, 100).finished()),
...
TargetQuadrotor::~TargetQuadrotor() {}

bool TargetQuadrotor::run(const Command &cmd, const Scalar ctl_dt) {
  if (!setCommand(cmd)) return false;
  return run(ctl_dt);
}

bool TargetQuadrotor::run(const Scalar ctl_dt) {
  ...
  while (remain_ctl_dt > 0.0) {
    ...
    // // Quadrotor velocity w.r.t. body frame
    // auto inertial_to_body = state_.q().toRotationMatrix().inverse();
    // const Vector<3> velocity_b = inertial_to_body * state_.v;
...
void TargetQuadrotor::init(void) {
  // reset
...
bool TargetQuadrotor::reset(const QuadState &state) {
...
Vector<4> TargetQuadrotor::runFlightCtl(const Scalar sim_dt, const Vector<3> &omega, const Command &command) {
...
void TargetQuadrotor::runMotors(const Scalar sim_dt, const Vector<4> &motor_thruts_des) {
...
bool TargetQuadrotor::setCommand(const Command &cmd) {
...
bool TargetQuadrotor::setState(const QuadState &state) {
  if (!state.valid()) return false;
  state_ = state;
  return true;
}

bool TargetQuadrotor::setWorldBox(const Ref<Matrix<3, 2>> box) {
...
bool TargetQuadrotor::constrainInWorldBox(const QuadState &old_state) {
...
Vector<3> TargetQuadrotor::quaternionToEuler(QuadState& state) const {
...
void TargetQuadrotor::setPIDControllerGain(const Scalar kp_vxy, const Scalar ki_vxy, const Scalar kd_vxy,
...
bool TargetQuadrotor::getState(QuadState *const state) const {
  if (!state_.valid()) return false;
...
bool TargetQuadrotor::getMotorThrusts(Ref<Vector<4>> motor_thrusts) const {
  motor_thrusts = motor_thrusts_;
  return true;
}

bool TargetQuadrotor::getMotorOmega(Ref<Vector<4>> motor_omega) const {
  motor_omega = motor_omega_;
  return true;
}

bool TargetQuadrotor::getDynamics(QuadrotorDynamics *const dynamics) const {
  if (!dynamics_.valid()) return false;
  *dynamics = dynamics_;
  return true;
}

const QuadrotorDynamics &TargetQuadrotor::getDynamics() { return dynamics_; }

bool TargetQuadrotor::updateDynamics(const QuadrotorDynamics &dynamics) {
  if (!dynamics.valid()) {
...
bool TargetQuadrotor::addRGBCamera(std::shared_ptr<RGBCamera> camera) {
  rgb_cameras_.push_back(camera);
  return true;
}

Vector<3> TargetQuadrotor::getSize(void) const { return size_; }

Vector<3> TargetQuadrotor::getPosition(void) const { return state_.p; }

std::vector<std::shared_ptr<RGBCamera>> TargetQuadrotor::getCameras(void) const {
  return rgb_cameras_;
};

bool TargetQuadrotor::getCamera(const size_t cam_id, std::shared_ptr<RGBCamera> camera) const {
...
bool TargetQuadrotor::getCollision() const { return collision_; }
